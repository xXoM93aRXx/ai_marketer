    ...
    <div style="font-family: Arial, sans-serif; line-height: 1.6; color: #333;">
      <div style="max-width: 600px; margin: 20px auto; border: 1px solid #ddd; border-radius: 8px; overflow: hidden;">
        <div style="background-color: #0077cc; padding: 20px; text-align: center;">
          <h1 style="color: #fff; font-size: 24px;">AI Marketer</h1>
        </div>
        <div style="padding: 20px;">
          <h2 style="font-size: 20px; margin-bottom: 10px;">${title}</h2>
          <p style="font-size: 16px; margin-bottom: 20px;">${message}</p>
          <div style="text-align: center;">
            <a href="${link}" style="background-color: #0077cc; color: #fff; text-decoration: none; padding: 10px 20px; border-radius: 4px; display: inline-block;">
              ${buttonText}
            </a>
          </div>
        </div>
...
async function sendConfirmationEmail({ to, name = "", token }) {
  ...
  const message = {
    ...
    html: generateEmailTemplate({
      ...
      message: `Please click the button below to verify your email address for AI Marketer.`,
      link: verifyLink,
...
async function sendPasswordSetupEmail({ to, token }) {
  ...
  const message = {
    ...
    html: generateEmailTemplate({
      ...
      message: "Click the button below to set your password for your AI Marketer account.",
      link: link,
